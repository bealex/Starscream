...
public class FoundationTransport: NSObject, Transport, StreamDelegate {
    ...
    public func connect(url: URL, timeout: Double = 10, certificatePinning: CertificatePinning? = nil) {
        ...
        workQueue.asyncAfter(deadline: .now() + timeout) { [weak self] in
            ...
            if !transport.isOpen {
                transport.delegate?.connectionChanged(state: .failed(FoundationTransportError.timeout))
            }
        }
    }
    ...
    private func getSecurityData() -> (SecTrust?, String?) {
        ...
        let propertyKey = CFStreamPropertyKey(rawValue: kCFStreamPropertySSLContext)
        let context = CFWriteStreamCopyProperty(outputStream, propertyKey) as! SSLContext?
        if domain == nil, let sslContextOut = context {
            ...
            if let peerDomain = String(bytes: peerName, encoding: .utf8), !peerDomain.isEmpty {
                domain = peerDomain
            }
        }
        return (trust, domain)
