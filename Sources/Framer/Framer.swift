...
private let opCodeMask: UInt8       = 0x0F
private let rsvMask: UInt8          = 0x70
private let rsv1Mask: UInt8         = 0x40
private let maskMask: UInt8         = 0x80
private let payloadLenMask: UInt8   = 0x7F
private let maxFrameSize: Int       = 32
...
public class WSFramer: Framer {
               ...
               let event = self?.process() ?? .needsMoreData
                switch event {
                    ...
                    case .processedFrame(let frame, let split):
                        guard let framer = self else { return }

                        framer.delegate?.frameProcessed(event: .frame(frame))
                        if split >= framer.buffer.count {
                            framer.buffer = Data()
                            return
                        }
                        framer.buffer = framer.buffer.advanced(by: split)
    ...
    private func process() -> ProcessEvent {
        ...
        if isControlFrame && payloadLen > 125 {
            let message = "payload length is longer than allowed for a control frame"
            let error = WSError(type: .protocolError, message: message, code: CloseCode.protocolError.rawValue)
            return .failed(error)
        }
    ...
    public func createWriteFrame(opcode: FrameOpCode, payload: Data, isCompressed: Bool) -> Data {
        ...
        if !isServer {
            ...
            for index in 0 ..< payloadLength {
                pointer[offset] = payload[index] ^ pointer[maskStart + (index % MemoryLayout<UInt32>.size)]
                offset += 1
            }
        } else {
            for index in 0 ..< payloadLength {
                pointer[offset] = payload[index]
                offset += 1
            }
        }
...
public extension Array where Element: MyWSArrayType & UnsignedInteger {
    ...
    func unmaskData(maskStart: Int, offset: Int, length: Int) -> Data {
        ...
        for index in 0 ..< length {
            unmaskedBytes[index] = UInt8(self[offset + index] ^ self[maskStart + (index % maskSize)])
        }
...
public func writeUint32( _ buffer: inout [UInt8], offset: Int, value: UInt32) {
    for index in 0 ... 3 {
        buffer[offset + index] = UInt8((value >> (8 * UInt32(3 - index))) & 0xff)
    }
}
...
public func writeUint64( _ buffer: inout [UInt8], offset: Int, value: UInt64) {
    for index in 0 ... 7 {
        buffer[offset + index] = UInt8((value >> (8 * UInt64(7 - index))) & 0xff)
    }
}
