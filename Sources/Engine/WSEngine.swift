...
public class WSEngine: Engine, TransportEventClient, FramerEventClient, FrameCollectorDelegate, HTTPHandlerDelegate {
    ...
    public func stop(closeCode: UInt16 = CloseCode.normal.rawValue) {
        ...
        write(data: payload, opcode: .connectionClose) { [weak self] in
            self?.reset()
            self?.forceStop()
        }
    }
    ...
    public func write(data: Data, opcode: FrameOpCode, completion: (() -> Void)?) {
        writeQueue.async { [weak self] in
            ...
            var sendData = data
            if let compressedData = engine.compressionHandler?.compress(data: data) {
                sendData = compressedData
                isCompressed = true
            }

            let frameData = engine.framer.createWriteFrame(opcode: opcode, payload: sendData, isCompressed: isCompressed)
            engine.transport.write(data: frameData) { _ in
                completion?()
            }
        }
    }
        ...
        switch state {
            case .connected:
                ...
                let wsReq = HTTPWSHeader.createUpgrade(
                    request: request, supportsCompression: framer.supportsCompression(), secKeyValue: secKeyValue
                )
                let data = httpHandler.convert(request: wsReq)
                transport.write(data: data) { _ in }
            case .waiting:
                break
